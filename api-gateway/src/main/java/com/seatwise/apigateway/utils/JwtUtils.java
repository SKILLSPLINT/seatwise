package com.seatwise.apigateway.utils;
    ...
    public ERole extractRole(String token) {
        try {
            ...
            if (roleObj instanceof String) {
                return ERole.valueOf((String) roleObj);
            } else if (roleObj instanceof ERole) {
                return (ERole) roleObj;
            }
            return ERole.USER; // Default role
        } catch (Exception e) {
            return ERole.USER; // Default role on error
        }
    }

    public String generateToken(ERole role, String email) {
        Map<String, Object> claims = new HashMap<>();
        claims.put("role", role);
        return generateRefreshToken(claims, email);
    }

    public String generateRefreshToken(Map<String, Object> extraClaims, String email) {
        return Jwts
                .builder()
                .claims(extraClaims)
                .subject(email)
